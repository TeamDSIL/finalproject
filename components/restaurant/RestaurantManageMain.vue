<template>
  <v-container>
    ...
    <v-row>
      <v-col cols="12">
        <v-text-field v-model="message" label="메시지 입력"></v-text-field>
      </v-col>
      <v-col cols="12">
...
export default {
  ...
  methods: {
    ...
    async sendMessage() {
      try {
        const response = await axios.post('http://localhost:8000/api/clova/reservation', {
          userRequest: {
            utterance: this.message
          }
        });
        this.responseMessage = response.data.template.outputs[0].simpleText.text;
      } catch (error) {
        console.error(error);
        this.responseMessage = '에러가 발생했습니다.';
      }
    },
